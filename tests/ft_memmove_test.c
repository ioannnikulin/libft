...
/*   Updated: 2023/11/14 19:08:29 by inikulin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_memmove_test(void)
{
	...
	/*
	t[6] = (t_testcase){10, "abc", 1, 2, 6, 1};
	t[7] = (t_testcase){10, "abc", 1, 2, 6, 2};
	t[8] = (t_testcase){10, "abc", 1, 2, 6, 3};
	*/
	for (int i = 0; i < SZ; i ++)
	{
		...
		char * custom_res = ft_memmove( \
			...
			t[i].tocopy);
		if (t[i].nullify == 0)
		{ // standard memmove segfaults on null
